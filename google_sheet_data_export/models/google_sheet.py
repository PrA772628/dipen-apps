...
class GoogleSpreadsheetIExport(models.Model):
        ...
        for data in final_record:
            for key,value in data.items():
                if isinstance(value,datetime.date):
                    data.update({
                        key : pd.to_datetime(value).strftime('%Y-%m-%d'),
                    })
    ...
    def run_single_sheet(self):
        ...
        email = data.get_param('google_sheet_data_export.email')
        p12_json_key = data.get_param('google_sheet_data_export.p12_json_key')
